...
/// Player-controlled plane with physics and fuel management
class PlanePlayer extends PositionComponent with HasGameRef<AviaRollHighGame>, CollisionCallbacks {
  PlanePlayer({
  ...
  // Plane state
  double _pressure = 50.0; // Start at 50% pressure (fuel)
  ...
  // Visual
  SpriteComponent? _planeSprite;
  double _currentRadius = GameConstants.planeBaseRadius;
  ...
  Future<void> onLoad() async {
    ...
    final stats = GameConstants.planeTypes[planeType]!;
    ...
    final spritePath = _getPlaneSpritePath();
    try {
      ...
      add(_planeSprite!);
      debugPrint('🎈 Balloon sprite loaded: $planeType from $spritePath');
    } catch (e) {
    ...
    debugPrint('🎈 Plane loaded: $planeType at ${position.x}, ${position.y}');
  }

  String _getPlaneSpritePath() {
    ...
    switch (planeType) {
      case PlaneType.standard:
        return 'planes/standart_plane.png'; // Note: keeping original typo from asset filename
      case PlaneType.foil:
      case PlaneType.fighter:
        return 'planes/sopwith_plane.png'; // WWI biplane
      case PlaneType.hydrogen:
      case PlaneType.jet:
        return 'planes/p-51.png'; // WWII fighter
      case PlaneType.cluster:
      case PlaneType.cargo:
        return 'planes/f-35.png'; // Modern jet
    }
  }
  ...
  void _updatePressure(double dt) {
    ...
    // Get stats for this plane type
    final stats = GameConstants.planeStats[planeType.name]!;
  ...
  void _updatePhysics(double dt) {
    ...
    final stats = GameConstants.balloonStats[planeType.name]!;
  ...
  void _updateVisualSize() {
    ...
    if (_planeSprite != null) {
      ...
      _planeSprite!.position = Vector2(offsetX, offsetY);
    }
  ...
  void _burst() {
    ...
    _isBurst = true;
    debugPrint('💥 Aircraft crashed at ${pressurePercent.toStringAsFixed(1)}% fuel!');
  ...
  void _stall() {
    ...
    _isStalled = true;
    debugPrint('🪂 Aircraft stalled! Out of fuel...');
  ...
  void onCollision(Set<Vector2> intersectionPoints, PositionComponent other) {
    ...
    if (otherType.contains('Obstacle') || 
      ...
      debugPrint('💥 HIT OBSTACLE: $otherType - AIRCRAFT CRASHED!');
  ...
  void render(Canvas canvas) {
    ...
    if (_planeSprite == null) {
      // Draw aircraft as colored circle (placeholder)
      final paint = Paint()
        ..color = pressureColor
